...
:::note
Note that you can always switch from **Standalone** to **Integrated Monorepo** later on by moving the application to the `apps` directory using the `@nx/workspace:move` generator.
...
### Merge a new Nx workspace into an existing repository
...
git remote add nx <path|url>

# Merge the new Nx workspace into the existing repository.
git fetch nx
git merge nx/main --allow-unrelated-histories
```
