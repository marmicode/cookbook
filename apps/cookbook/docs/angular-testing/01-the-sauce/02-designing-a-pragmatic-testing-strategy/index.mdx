...
Tests _([Narrow](/angular/testing/glossary#narrow-tests) and [Wide](/angular/testing/glossary#wide-tests))_ are safety nets among others like [Dogfooding](/angular/testing/glossary#dogfooding) or [Canary Release](/angular/testing/glossary#canary-release).
...
A common oversight when designing a testing strategy is to mainly focus on [Wide tests](/angular/testing/glossary#wide-tests) in general. Things get even worse when the model is topped with a few calories of manual testing.
...
- The test pyramid bottom layer is generally defined as "unit tests", but as mentioned in the [previous chapter](/angular/testing/beyond-unit-vs-integration), the "unit test" term is confusing and often misinterpreted.
...
- **[Over-Specifying](/angular/testing/glossary#over-specification)**: They are too structure-sensitive and coupled to implementation details. They do not survive refactoring.
- **Expensive to Implement**: They require substantial test setup and [test double](/angular/testing/glossary#test-doubles) orchestration.
- **Expensive to Maintain**: Being over-specifying makes them expensive to maintain.
- **Not [Predictive](/angular/testing/glossary#predictive)**: They are so [asymmetric to production](/angular/testing/glossary#symmetric-to-production) that they are not predictive of the system's behavior. All the tests pass, but the feature is actually broken.
...
<MegaQuote>
  ...
  definition](/angular/testing/beyond-unit-vs-integration#narrow-tests): fast,
...
Interestingly, in the example above, we can observe that with such an approach, we can reach a relatively high level of confidence with only a few tests (<span style={{color: 'orange'}}>**1 Wide**</span>, <span style={{color: '#1298fb'}}>**2 Narrow**</span>, <span style={{color: 'green'}}>**1 narrower**</span>) and one Test Double. Our tests are [**composable**](/angular/testing/glossary#composable).
...
While you might think that the answer to all these examples is obvious, it is actually subjective. The answer depends on the context of your product, the phase of the product _(Cf. [3X](/angular/testing/glossary#3x))_, the team, and the users _(their nature and volume)_, among other things.

### The Testing Strategy Should Be Part of the Design Doc

When designing a new feature, the testing strategy for that specific feature should be part of the [Design Doc](/angular/testing/glossary#design-doc) _(or whatever drawings or notes you make before coding)_. The testing strategy should be as important as the architecture.
...
I had the privilege of being "born" into [eXtreme Programming](/angular/testing/glossary#extreme-programming) _(XP)_. My first job _(back in 2007)_ was with a team deeply committed to XP values, principles, and practices. As the least experienced developer on the team, [Test-Driven Development](/angular/testing/glossary#tdd) _(TDD)_ saved me _(among other practices)_.

Thirteen years later, having always relied on testing and TDD, **I had to challenge my well-established assumptions**. What if we didn't need as much testing? Could there be situations where testing wasn't necessary at all? We don't need tests for a one-shot simple script or some [Spikes](/angular/testing/glossary#spike), but we need tests for ambitious products, right? But where is the line?
...
- **🚨 Number of [False Positives](/angular/testing/glossary#false-positive)**
- **🥷 Number of [False Negatives](/angular/testing/glossary#false-negative)**
